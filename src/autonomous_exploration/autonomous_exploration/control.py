...
class navigationControl(Node):
    def __init__(self):
        ...
        self.goal_sub = self.create_subscription(Bool, "/goal_reached", self.goal_reached_callback, 10)
    ...
    def goal_reached_callback(self, msg):
        if msg.data:
            print("[BILGI] GOAL REACHED, RETURNING TO ORIGIN!")
            self.goal_reached = True
            self.publish_origin_goal()

    def publish_origin_goal(self):
        goal = PoseStamped()
        ...
        print("[BILGI] Published Nav2 goal at (0, 0, 0) in map frame")

    def exp(self):
        twist = Twist()
        while rclpy.ok() and not self.goal_reached: #Sensor verileri gelene kadar bekle.
        ...
        if self.goal_reached:
            print("[BILGI] Exploration stopped due to goal reached signal.")
